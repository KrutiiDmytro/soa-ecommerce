...
- **Verify ✅:** усі 6 сервісів `healthy`; unit 98 і integration 37 зелені на оновленій
  ...
  ⚠️ Сам пайплайн зеленим не показаний: до репозиторію не підключено раннер — усі його кроки
  виконані вручну локально, але перевірити CI можна лише після призначення раннера.
