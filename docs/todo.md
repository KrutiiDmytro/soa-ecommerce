...
- [x] Раннер #303 — shell-типу, всі джоби ділять одну робочу теку: `unit` більше не монтує
      компонент на запис, а копіює його в контейнер (`:ro` + `cp -a`), `integration.after_script`
      повертає власника теки, плюс автоматична стадія `prepare:workspace` перед усіма
      іншими — пайплайн розблоковує теку сам, без ручного втручання.
- [x] `composer.lock` виведено з `.gitignore` і закомічено (7 пакетів). Доти CI резолвив
      залежності з нуля на кожній джобі: прогін був недетермінований, і будь-який реліз
      Symfony 7.4.x чи PHPUnit 11.x зламав би збірку без жодної зміни в коді. Нова джоба
      `lint:composer` стереже єдину ваду схеми — `composer.json` змінили, лок ні.
- [x] Три воркери чекають на `condition: service_healthy` свого сервісу. Вони монтують ту
      саму теку, що й він, і спільний `entrypoint` у них теж робить `composer install`;
      за спискової форми `depends_on` (= `service_started`) обидва бралися за один `vendor/`
      одночасно — подвійна робота і гонка двох composer в одній теці. Найпомітніше це на
      чистій теці CI, де `vendor/` доводиться ставити з нуля.
- [x] `integration` ставить залежності один раз до `docker compose up`, а не дев'ять разів
      у контейнерах на їхньому старті. На 1 vCPU саме та черга з'їдала півгодини, і джоба
      впиралась у таймаут (пайплайн #21865: 30 хв, стек так і не піднявся). Обхід
      `--entrypoint composer` обов'язковий: інакше `entrypoint.sh` зробив би свій
      `composer install` ще до нашого, і установка вийшла б подвійною.
- [x] Мердж `develop` → `main` як фінальний реліз. Доти `main` стояв на Фазі 3, тобто за
      посиланням на репозиторій було видно проєкт без ESB, тестів і CI — усе зроблене
      лежало в `develop`.
...
- **Фаза 9** (2026-08-01, гілка `feat/phase9-docs-ci`). Документація, CI, інфраструктурні борги.
  ...
  - **Shell-раннер отруюється власними джобами.** #303 не docker-in-docker, а shell: усі джоби
    ділять одну робочу теку. `composer install` усередині контейнера йде від root і лишав у ній
    root-owned `vendor/`, після чого `git clean` наступної джоби (від `gitlab-runner`) падав із
    `Permission denied` **ще до `script`**. У матриці `unit` це виглядало як «пройшов лише перший
    компонент, решта п'ять червоні» — тести навіть не стартували. Три відповіді: `unit` монтує
    джерела `:ro` і копіює їх у `/app` (нічого не пишеться на хост), `integration.after_script`
    першим кроком робить `chown` назад, а `prepare:workspace` розблоковує теку на початку
    кожного прогону. Останнє тримається на `GIT_STRATEGY: none` — без нього джоба падала б
    на тому самому `git clean`, який і має полагодити. Стадія окрема й перша саме тому, що
    в межах однієї стадії джоби йдуть паралельно: `lint` устиг би впасти раніше за чистку.
    Спільний раннер = спільний стан: у CI це така сама змінна, як БД у тестах.
