...
PHP 8.2 / Symfony 7.4 · native `SoapServer` · Postgres-per-service · RabbitMQ · Redis · Docker Compose
· **143 тести** (98 unit / 37 integration / 8 e2e)

> Дизайн, декомпозиція та контракти: репозиторій **Task 27**. Тут — виконання.
> План, хід робіт, ухвалені рішення та уроки: [`docs/todo.md`](docs/todo.md).

> **Статус: завершено й заморожено (серпень 2026).** Останній прогін CI зелений —
> 11 джоб за 2:29. Пайплайн вимкнено навмисно: раннер стоїть на дроплеті 1 vCPU / 1 ГБ
> разом із чужим продом, і підстав відбирати в нього ресурси більше немає. Локально
> проєкт піднімається й проганяє всі три рівні тестів, як описано нижче.

## Найкоротший шлях подивитись

```bash
cp .env.example .env && docker compose up -d
docker compose run --rm e2e     # наскрізний бізнес-процес через шину
```

Якщо часу зовсім мало — читайте таблицю нижче: вона показує, де саме в коді видно SOA.
Клієнт у цьому проєкті знає **лише адресу ESB**, і `tests/` це доводить: жоден e2e-тест
не має адреси сервісу.
...
Разом `docker compose up` піднімає **15 контейнерів**.
...
Залежності й прогрів кешу робить спільний [`docker/entrypoint.sh`](docker/entrypoint.sh) при старті
контейнера, тому сервіс відкриває порт не миттєво — готовність показує `docker compose ps` (`healthy`).
Воркери чекають на `healthy` свого сервісу: вони монтують ту саму теку, і без цього два
`composer install` бралися б за один `vendor/` одночасно.
...
| Рівень | Скільки | Що перевіряє |
|---|---|---|
| unit | 98 | домен ізольовано — без БД, брокера й мережі |
| integration | 37 | справжній SOAP-виклик до піднятого сервісу + реальна БД |
| e2e | 8 | повний потік через ESB; клієнт знає лише адресу шини |

```bash
# Unit — стек не потрібен
docker compose exec order-management php vendor/bin/phpunit

# Integration — потрібен піднятий стек
docker compose exec order-management php vendor/bin/phpunit --testsuite integration

# E2E — окремий пакет tests/
docker compose run --rm e2e
```

## CI

[`.gitlab-ci.yml`](.gitlab-ci.yml): `prepare` → `lint` → `unit` → `integration`.

| Стадія | Джоби | Коли |
|---|---|---|
| `prepare` | розблокування робочих тек раннера | автоматично |
| `lint` | синтаксис PHP · валідність WSDL/XSD як XML · синхрон `composer.json` ↔ `composer.lock` | автоматично |
| `unit` | матриця по 6 компонентах | автоматично |
| `integration` | піднятий стек, міграції, seed, integration-сюїти + e2e | **вручну** |

`composer.lock` усіх семи пакетів закомічено, тож CI ставить залежності з локів, а не резолвить
щоразу заново — інакше зелений прогін залежав би від того, що опубліковано в packagist сьогодні.

`integration` лишається ручною свідомо: доступний раннер стоїть на дроплеті 1 vCPU / 1 ГБ разом
із чужим продом, і автоматичний підйом 15 контейнерів означав би OOM не джоби, а сусіда.
**Рівні integration та e2e підтверджені локально, а не пайплайном** — це чесне обмеження, а не
недогляд.
...
docker/        спільний образ PHP 8.2 (soap, pdo_pgsql, redis, amqp) + entrypoint
